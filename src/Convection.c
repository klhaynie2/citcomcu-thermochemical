...
void convection_initial_fields(struct All_variables *E)
{
	...
	if(E->control.composition)
	{
		...
		fprintf(stderr, "aaaa %d %g %g Proc: %d\n", E->advection.markers, E->lmesh.volume, E->mesh.volume, E->parallel.me); // KLH 1/14/2019 also  print out processor number for the corresponding marker and local mesh volume information
...
void convection_initial_temperature(struct All_variables *E)
{
  ...
  if (E->control.restart == 0)
    {
      if(read_new_field(E,E->T,"temperature")==0){
	fprintf(E->fp,"Using internally defined temperature field \n");
	...
	if(E->control.CART3D)
		...
		for(k = 1; k <= noz; k++)
		  {
		    ...
		    z1 = E->X[3][node];
		    y1 = E->X[2][node];
		    E->T[node] = 1 - z1 + E->convection.perturb_mag[p] * sin(M_PI * (1.0 - z1)) * 
	...
	else if(E->control.Rsphere)
	  {
		...
		for(k = 1; k <= noz; k++)
		  {
		    ...
		    */
		    ...
		    E->T[node] = beta * (1.0 - 1.0 / z1) + E->convection.perturb_mag[p] * 
		      modified_plgndr_a(ll, mm, x1) * cos(mm * y1) * sin(M_PI * (z1 - E->sphere.ri) / 
		    ...
		    E->T[node] = beta * (1.0 - 1.0 / z1) + E->convection.perturb_mag[p] * 
		      ...
		      ((E->mesh.nsd != 3) ? 1.0 : cos(E->convection.perturb_k[p] * M_PI * 
						      (y1 - E->XG1[2]) / (E->XG2[2] - E->XG1[2])));
        ...
        // Read in input composition field - KLH 3.6.19:
        read_new_field(E,E->C,"composition");
  ...
  if ( (fp = fopen(output_file,"w")) != NULL) {
	...
	fprintf(fp,"LAT[%06d] = %.4e LON[%06d] = %.4e RAD[%06d] = %.4e T[%06d] = %.4e \n",j,
		E->SX[1][j],j,E->SX[2][j],j,E->SX[3][j],j,E->T[j]);
    else 
      for (j=1;j<=E->lmesh.nno;j++)
	fprintf(fp,"X[%06d] = %.4e Y[%06d] = %.4e Z[%06d] = %.4e T[%06d] = %.4e \n",j,
		E->X[1][j],j,E->X[2][j],j,E->X[3][j],j,E->T[j]);
  }        
...
/* ==================================================================================
 Changes to convection_initial_markers were made to read in an initial tracer
 field with a location and value of 0 (ambient) or 1 (anomalous).
 The original method of assigning tracers a value of
 0 or 1 based on depth has been commented out.
 
 KLH November 13, 2018
===================================================================================== */

void convection_initial_markers(struct All_variables *E)
{
	//int el, i, j, k, p, node, ii, jj;
	int el, notracer, node;
	//double x, y, z, r, t, f, dX[4], dx, dr;
	double x, y, z, r, t, f, dX[4];
	//char input_s[100], output_file[255];
	//FILE *fp;
    // Below added in for composition
    int i,j, jj,kk,el1;
    double ttt, ttf, ttr, tx, ty, tz, tr, tf, tt; /// for getting time for testing - KLH 2/22/2019
    float temp, *CI;
    const int dims = E->mesh.nsd;
    const int ends = enodes[dims];
    const int vpts = vpoints[E->mesh.nsd];
    
    // Allocate memory and initialze CI (for input composition at nodes):
    CI = (float *)malloc((2+E->lmesh.nno)*sizeof(float));
    for (i = 1; i <= E->lmesh.nno; i++) {
        CI[i] = 0.0;
    }
    // Read in input composition field:
    read_new_field(E,CI,"composition");
    fprintf(stderr,"Finished reading in Composition field on processor = %d  with CPU time = %0.4e \n",E->parallel.me,((CPU_time0() - E->monitor.begin_time)/3600));
//    fprintf(stderr,"Finished reading in Composition field now assigning tracers with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600));   // KLH 02/20/2019 added for testing
	if(E->control.CART3D)
	{
		notracer = 0;
		do
		{
			...
			if((x >= E->XP[1][1] && x <= E->XP[1][E->lmesh.nox]) && (y >= E->XP[2][1] && y <= E->XP[2][E->lmesh.noy]) && (z >= E->XP[3][1] && z <= E->XP[3][E->lmesh.noz]))
			{
				...
				el = get_element(E, E->XMC[1][notracer], E->XMC[2][notracer], E->XMC[3][notracer], dX);
				E->CElement[notracer] = el;
 //               fprintf(stderr,"Randomly Distributed Tracers in Each Element...with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600)); // KLH 02/20/2019 added for testing
            // Below is for composition based on depth (commented out):
            /* if(E->XMC[3][notracer] > E->viscosity.zcomp)
                 E->C12[node] = 0;
               else
                 E->C12[node] = 1; */
                
                // Assign nodal composition from input composition file to elements:
                    for(jj = 1; jj <= vpts; jj++) {
                        for(kk = 1; kk <= ends; kk++) {
                            E->C[(el-1)*vpts +jj] += CI[E->ien[el].node[kk]]*E->N.vpt[GNVINDEX(kk,jj)];
                       }
                    temp =  E->C[(el-1)*vpts +jj];
//		  fprintf(stderr,"Assigned Nodal Composition From Input to Elements...with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600)); // KLH 02/20/2019 added for testing
                    if(temp > 0.5)
                        E->C12[notracer] = 1; // if elemental composition is >0.5 assign tracers within that element a value of 1 (anomalous)
                    else
                        E->C12[notracer] = 0; // else assign tracers within that element a value of 0 (ambient)
                    } // jj
			} // if
		} while(notracer < E->advection.markers); // do
//		fprintf(stderr,"Assigned Each Tracer a Composition Value Based on Elements with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600));  // KLH 02/20/2019 added for testing
	} // cart3D
	else if(E->control.Rsphere)
	{
		notracer = 0;
		do
		{
			x = (drand48() - 0.5) * 2.0;
            if(E->parallel.me == 0){
             tx = ((CPU_time0() - E->monitor.begin_time)/3600);
            }
             fprintf(stderr,"Finished with x....\n");
			y = drand48();
            if(E->parallel.me == 0){
             ty = ((CPU_time0() - E->monitor.begin_time)/3600);
            }
             fprintf(stderr,"Finished with y....\n");
//       y = (drand48()-0.5)*2.0;
			z = (drand48() - 0.5) * 2.0;
            if(E->parallel.me == 0){
             tz = ((CPU_time0() - E->monitor.begin_time)/3600);
            }
             fprintf(stderr,"Finished with z....\n");
			r = sqrt(x * x + y * y + z * z);
            if(E->parallel.me == 0){
             tr = ((CPU_time0() - E->monitor.begin_time)/3600);
            }
             fprintf(stderr,"Finished with r....\n");
			t = acos(z / r);
            if(E->parallel.me == 0){
             tt = ((CPU_time0() - E->monitor.begin_time)/3600);
            }
             fprintf(stderr,"Finished with t....\n");
			f = myatan(y, x);
            if(E->parallel.me == 0){
             tf = ((CPU_time0() - E->monitor.begin_time)/3600);
            }
             fprintf(stderr,"Finished with f....\n");
            
            if(E->parallel.me == 0){
                fprintf(stderr,"CPU time for random tracer coordinates: x = %0.4e, y = %0.4e, z = %0.4e, tr = %0.4e, tt = %0.4e, tf= %0.4e\n", tx, ty, tz, tr, tt, tf);
            }
			...
			if((t >= E->XP[1][1] && t <= E->XP[1][E->lmesh.nox]) && (f >= E->XP[2][1] && f <= E->XP[2][E->lmesh.noy]) && (r >= E->XP[3][1] && r <= E->XP[3][E->lmesh.noz]))
			{
				...
				E->XMC[1][notracer] = t;
                if(E->parallel.me == 0){
                ttt = ((CPU_time0() - E->monitor.begin_time)/3600);
                }
                fprintf(stderr,"Finished with assigning tracers to t....\n");
				E->XMC[2][notracer] = f;
                if(E->parallel.me == 0){
                ttf = ((CPU_time0() - E->monitor.begin_time)/3600);
                }
                fprintf(stderr,"Finished with assigning tracers to f....\n");
				E->XMC[3][notracer] = r;
                if(E->parallel.me == 0){
                ttr = ((CPU_time0() - E->monitor.begin_time)/3600);
                }
                fprintf(stderr,"Finished with assigning tracers to r....\n");
				el = get_element(E, E->XMC[1][notracer], E->XMC[2][notracer], E->XMC[3][notracer], dX);
				E->CElement[notracer] = el;
                if(E->parallel.me == 0){
               fprintf(stderr,"Randomly Distributed Tracers in Each Element...with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600)); // KLH 02/20/2019 added for testing
               fprintf(stderr,"CPU time for assigning tracer to coordinates: ttr = %0.4e, ttt = %0.4e, ttf= %0.4e\n", ttr, ttt, ttf);
                }
           // Below is for composition based on depth (commented out):
...
//		fprintf(stderr,"Assigned Nodal Composition From Input to Elements...with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600)); // KLH 02/20/2019 added for testing
		...
		} while(notracer < E->advection.markers);
        if(E->parallel.me == 0){
		fprintf(stderr,"Assigned Each Tracer a Composition Value Based on Elements with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600));  // KLH 02/20/2019 added for testing
        }
	}

//	fprintf(stderr,"Finished assiging tracers! Next, getting C from markerswith CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600)); // KLH 02/20/2019 added for testing
	get_C_from_markers(E, E->C);
    if(E->parallel.me == 0){
	fprintf(stderr,"Finished getting C from markers!with CPU time = %0.4e\n", ((CPU_time0() - E->monitor.begin_time)/3600)); // KLH 02/20/2019 added for testing
    }
	return;
}
