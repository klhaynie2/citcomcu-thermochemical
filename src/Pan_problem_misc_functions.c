...
/* ============================================================================
   Function to read in an INITIAL scalar mesh field with a single value at each 
   mesh point on the global mesh and assign it to the appropriate variable on
   the local (processor) mesh.

   This is basically a modified version of read_previous_field which reads in a
   single column binary file.

   MIB JAN07
   ============================================================================ */

int read_new_field(struct All_variables *E, float *field, char *name)
{
  char *filename, *input_token;
  FILE *fp;
  int i, j, k, size2, nodel, nodeg;
  float *intemp;

  /* Get filename from input file information */
  filename = (char *)malloc(1000 * sizeof(char));
  input_token = (char *)malloc(1000 * sizeof(char));
  sprintf(input_token, "new_%s_file", name);
 
  if(!input_string(input_token, filename, "initialize")) {
    fprintf(E->fp, "No new %s information found in input file %s\n", name,filename);
    fflush(E->fp);
    return (0);	  /* if not found, take no further action, return zero */
  }
  fprintf(E->fp, "New %s information is in file %s\n", name, filename);
  fflush(E->fp);
  ...
  /* File format is binary one float at each global mesh node */
  size2 = E->mesh.nno*sizeof(float);
  intemp = (float *) malloc(size2);
  fread(intemp,size2,1,fp);
  fclose(fp);

  for (k=1;k<=E->lmesh.noy;k++) {
      for (i=1;i<=E->lmesh.nox;i++) {
	for (j=1;j<=E->lmesh.noz;j++) {
	  nodel = j + (i - 1)*E->lmesh.noz + (k - 1)*E->lmesh.noz*E->lmesh.nox;
	  nodeg = (j - 1 + E->lmesh.nzs) + (i - 2 + E->lmesh.nxs)*E->mesh.noz + 
	      (k - 2 + E->lmesh.nys)*E->mesh.noz*E->mesh.nox; 
	  field[nodel] = intemp[nodeg-1]; /* intemp starts at 0 */
	  //  fprintf(stderr,"k %d i %d j %d nodel %d nodeg-1 %d T[nodel] %e T[nodeg-1] %e\n",k,i,j,nodel,nodeg-1,E->T[nodel],intemp[nodeg-1]);
	    if (strcmp(name,"temperature") == 0){
	      E->C[nodel] = 0;
	      E->node[nodel] = E->node[nodel] | (INTX | INTZ | INTY);
	    }
