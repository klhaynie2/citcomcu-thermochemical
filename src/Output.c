...
/* ==========================================================================  
   ...
   Also Outputs tracer coordinates and composition - KLH JAN2019
   ========================================================================== */
void output_velo_related_binary(struct All_variables *E, int file_number)
{
	...
	static float *SV, *TX, *TY, *TZ, *TC, *visc, *pres, *vtyp, *edot;
	static int been_here = 0;
	
	if (been_here==0)
	{
		SV = (float *) malloc((3*E->lmesh.nnov+1)*sizeof(float));
        TX = (float *) malloc((E->advection.markers+1)*sizeof(float));
        TY = (float *) malloc((E->advection.markers+1)*sizeof(float));
        TZ = (float *) malloc((E->advection.markers+1)*sizeof(float));
        TC = (float *) malloc((E->advection.markers+1)*sizeof(float));
   ...
   for (i = 1; i<=E->advection.markers+1; i++){ // KLH 1/14/2019
            TX[i] = 0.0;
            TY[i] = 0.0;
            TZ[i] = 0.0;
            TC[i] = 0.0;
	...
	E->filed[14]=fopen(output_file,"w"); // moved out of if comp KLH 3.6.19
    
     if(E->control.composition) {
        sprintf(output_file, "%s.tracersX.%d.%d", E->control.data_file,E->parallel.me,file_number);
        E->filed[17] = fopen(output_file, "w");
        sprintf(output_file, "%s.tracersY.%d.%d", E->control.data_file,E->parallel.me,file_number);
        E->filed[18] = fopen(output_file, "w");
        sprintf(output_file, "%s.tracersZ.%d.%d", E->control.data_file,E->parallel.me,file_number);
        E->filed[19] = fopen(output_file, "w");
        sprintf(output_file, "%s.tracersC.%d.%d", E->control.data_file,E->parallel.me,file_number);
        E->filed[20] = fopen(output_file, "w");
	}
	...
	//if (E->control.composition) { // commented out KLH 3.6.19
	   size2 = (E->lmesh.nno+1)*sizeof(float);
	...
	// Below outputs just X, Y, Z, C12 for tracers at each time step and from each processor in binary - KLH 10/16/2018
	if(E->control.composition)
	{
	size2 = (E->advection.markers+1)*sizeof(float);
        get_tracersX(E,TX);
        fwrite(TX,size2,1,E->filed[17]);
        get_tracersY(E,TY);
        fwrite(TY,size2,1,E->filed[18]);
        get_tracersZ(E,TZ);
        fwrite(TZ,size2,1,E->filed[19]);
        get_tracersC(E,TC);
        fwrite(TC,size2,1,E->filed[20]);
...
/* ==========================================================================
 Output tracer inforamtion
 KLH JAN2019
 ========================================================================== */
void get_tracersX(struct All_variables *E, float *TX)
{
    int i;
    TX[0] = 0.0;
    
	TX[0] = E->advection.markers; // add in total number of markers for each local procresser
...
void get_tracersY(struct All_variables *E, float *TY)
{
    int i;
    TY[0] = 0.0;
    
	TY[0] = E->advection.markers;
    for (i=1;i<=E->advection.markers;i++)  {
        TY[i] = E->XMC[2][i];
    }
...
void get_tracersZ(struct All_variables *E, float *TZ)
{
    int i;
    TZ[0] = 0.0;
    
	TZ[0] = E->advection.markers;
    for (i=1;i<=E->advection.markers;i++)  {
        TZ[i] = E->XMC[3][i];
    }
...
void get_tracersC(struct All_variables *E, float *TC)
{
    int i;
    TC[0] = 0.0;
    
	TC[0] = E->advection.markers;
    for (i=1;i<=E->advection.markers;i++)  {
        TC[i] = E->C12[i];
    }
