...
void process_new_velocity(struct All_variables *E, int ii)
{
  ...
  if(((ii % E->control.record_every) == 0) || (ii == -1)) /* MIB, JAN07, added ii == -1*/
    {
      // get_CBF_topo(E,E->slice.tpg,E->slice.tpgb);  /* MIB JAN07: appears to output zeros?? */
      
      get_STD_topo(E, E->slice.tpg, E->slice.tpgb, ii); /* Also Calcs domain stress dstress */

      averages(E);
      
      // output_velo_related(E, ii);	/* also topo */ /* MIB JAN07 */
      output_velo_related_binary(E, ii); /* MIB JAN07 */
    }
