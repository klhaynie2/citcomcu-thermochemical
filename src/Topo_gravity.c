...
void get_STD_topo(struct All_variables *E, float *tpg, float *tpgb, int ii)
{
	...
	for(i = 1; i <= nno; i++)
	{
		...
		/* Save domain stress for output MIB JAN07 */
		E->dstress[(i-1)*6 + 1] = SXX[i];
		E->dstress[(i-1)*6 + 2] = SYY[i];
		E->dstress[(i-1)*6 + 3] = SZZ[i];
		E->dstress[(i-1)*6 + 4] = SXY[i];
		E->dstress[(i-1)*6 + 5] = SXZ[i];
		E->dstress[(i-1)*6 + 6] = SZY[i];
