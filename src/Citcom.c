...
/*	parallel_process_initialization(&E,argc,argv); */
	...
	E.monitor.begin_time = CPU_time0(); /* Added 9/9/12 MAJ. For timing on Lonestar. */
	...
	if(E.parallel.me == 0)
	{
		...
		if (E.control.stokes) /* if else statement added MAJ 11/12/2007.*/
		{
		...
		{
		fprintf(E.fp, "Average cpu time taken for velocity step = %f\n", time / ((float)(E.monitor.solution_cycles + 1))); /* MAJ 11/12/07 out "E.monitor.solution_cycles - 1" */
		fprintf(stderr, "Average cpu time taken for velocity step = %f\n", time / ((float)(E.monitor.solution_cycles + 1))); /* MAJ 11/12/07 out "E.monitor.solution_cycles - 1" */
		/* MAJ 11/12/07 replaced "E.monitor.solution_cycles-1" on two lines above with "E.monitor.solution_cycles+1". The "-1" likely typo in Citcom.c. because */
		/* E.monitor.solution_cycles starts ==0 in Citcom.c. Then calls general_stokes_solver subfunction. If stokes flow only it's incremented to ==1 else */
		/* If time-dependent problem E.monitor.solution_cycles orig 0 is incremented +1 on each time-dep pass.*/ 
		/* Thus, if stokes flow only or if one time-step: the old way "E.monitor.solution_cycles - 1" would equal 0, resulting in "time/0" which == infinity. */
		/* And if time-step > 1 then, average cpu time old way would be incorrect b/c it would divide by "n-1" passes not "n" passes through velocity solution. */
		/* And it would ignore the first general_stokes_solver call. */
		}
	}
