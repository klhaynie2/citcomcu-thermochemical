...
void allocate_common_vars(struct All_variables *E)
{
	...
	E->dstress = (float *)malloc((6*E->lmesh.nno + 1) * sizeof(float)); /* MIB JAN07 */
	E->visctyp = (float *)malloc((E->lmesh.nel + 1) * sizeof(float)); /* MIB, JAN07 */
...
void global_derived_values(struct All_variables *E)
{
	...
	for(i = E->mesh.levmax; i >= E->mesh.levmin; i--)	/* set up dimensions for different grids  */
	{
		...
		if(E->control.print_convergence && E->parallel.me == 0) {
			if(E->control.NMULTIGRID || E->control.EMULTIGRID) /*MAJ 7/26/07 */
			fprintf(stderr, "For i= %d Mesh lev has %d x %d x %d nodes\n",i, E->mesh.NOX[i], E->mesh.NOZ[i], E->mesh.NOY[i]);
		}
...
void read_initial_settings(struct All_variables *E)
{
    ...
    input_int("mesh_from_file",&(E->mesh.from_file), "0"); /* MIB JAN07 */
	input_string("mesh_file",E->mesh.meshfile, " "); /* MIB JAN07 */
...
//  input_float("layerd",&(E->data.layer_km),"2870000.0");
	...
	input_float("Tsurf", &(E->data.Tsurf), "273.0");
	...
	input_float("adiabatcomp",&(E->data.adiabat_comp),"4.3e-12"); /* MIB, JAN07 */
