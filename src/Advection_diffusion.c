	...
	if(FLAGS != NULL)
	{
		...
		for(a = 1; a <= ends; a++)
			if(FLAGS[E->ien[el].node[a]] & FBZ)
			{
				...
				nodes[4] = loc[loc[a].node_nebrs[0][0]].node_nebrs[2][1]; /*Changed from 2 to 1 1/24/08 MAJ*/
				nodes[3] = loc[loc[a].node_nebrs[0][1]].node_nebrs[2][1]; /*Changed from 2 to 1 1/24/08 MAJ*/
...
//		E->data.disptn_number = 1e3*E->data.therm_exp*E->data.grav_acc*E->data.radius_km/E->data.Cp;
	...
	if(E->parallel.me == 0)
	  fprintf(E->fp, "visc_heating temp6=%g\n", temp6);
	}
	...
	if(E->parallel.me == 0)
	  fprintf(E->fp, "adi_heating temp5=%g\n", temp5);
	}
	...
	if (E->control.visc_heating || E->control.adi_heating || E->control.Ra_670 != 0.0 || E->control.Ra_410 != 0 ) {
