...
void Runge_Kutta(struct All_variables *E, float *C, float *V[4], int on_off)
{
	...
	MPI_Allreduce(&E->advection.markers, &E->advection.markers_g, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD); // KLH FEB2019 to output sum of markers (total/global number of tracers) for every time-step as opposed to being output only when the reminder of a time-step divied by 10 is 0
...
//	fprintf(stderr,"In get C from markers - about to count anomalous and ambient tracers and use ratio method\n"); // KLH 02/20/2019 added for testing
...
//	fprintf(stderr,"Finished with ratio method!\n"); // KLH 02/20/2019 added for testing
	return;
}
